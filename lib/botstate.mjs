...
import { QUESTS, MODE_NAMES, TALENT_INFO, TALENT_CHOICE_NAMES, SPEC_NAMES, talentPointsAtLevel } from './gamedata.mjs';
import { ITEMS } from './items.generated.mjs';
import { xpToNext, mobName, questName, itemName as displayItemName, splitCoin, SLOT_NAMES, STAT_NAMES, KIND_NAMES } from './english.mjs';
...
const itemName = (id) => displayItemName(id, ITEMS[id]?.name ?? id);
const itemQ = (id) => ITEMS[id]?.quality ?? 'common';

// Bounded, credential-free world projection for the console's live tactical viewport.
// The server snapshot can contain more fields than the browser needs, so explicitly
// allowlist display data and keep only nearby entities.
export function tacticalView(world, radius = 45) {
  const self = world.self;
  if (!self || !Number.isFinite(self.x) || !Number.isFinite(self.z)) return null;
  const targetId = self.target;
  const entities = [...world.ents.values()]
    .filter((entity) => entity.id !== self.id && ['mob', 'player', 'npc', 'object'].includes(entity.k)
      && Number.isFinite(entity.x) && Number.isFinite(entity.z))
    .map((entity) => ({ entity, distance: Math.hypot(entity.x - self.x, entity.z - self.z) }))
    .filter(({ distance }) => distance <= radius)
    .sort((a, b) => a.distance - b.distance)
    .slice(0, 100)
    .map(({ entity }) => ({
      id: entity.id, kind: entity.k, x: entity.x, z: entity.z,
      name: entity.k === 'mob' ? mobName(entity.tid, entity.nm) : (entity.nm ?? entity.tid ?? entity.k),
      level: entity.lv, dead: !!entity.dead, loot: !!entity.loot,
      hpPct: entity.mhp > 0 ? Math.max(0, Math.min(1, entity.hp / entity.mhp)) : null,
      target: entity.id === targetId, aggro: entity.aggro === self.id,
    }));
  return { radius, self: { x: self.x, z: self.z, hpPct: self.mhp > 0 ? self.hp / self.mhp : 0 }, entities };
}
...
export function richState({ world, action, zone, session, settings, logBuf, online, host, name, cls }) {
  ...
  const stats = s.stats ?? {};
  const statsArr = [
    { k: STAT_NAMES.str, v: stats.str }, { k: STAT_NAMES.agi, v: stats.agi }, { k: STAT_NAMES.sta, v: stats.sta },
    { k: STAT_NAMES.int, v: stats.int }, { k: STAT_NAMES.spi, v: stats.spi }, { k: STAT_NAMES.armor, v: stats.armor },
    { k: 'Attack power', v: s.ap }, { k: 'Critical strike', v: s.crit != null ? (s.crit * 100).toFixed(1) + '%' : '—' },
  ].filter((x) => x.v != null);
  const equip = ['mainhand', 'chest', 'legs', 'feet'].map((slot) => { const id = s.equip?.[slot]; return { slot, slotName: SLOT_NAMES[slot], name: id ? itemName(id) : null, quality: id ? itemQ(id) : 'common' }; });
  const inv = (s.inv ?? []).map((it) => { const id = it.itemId ?? it.id; return { name: itemName(id), count: it.count ?? 1, quality: itemQ(id), kindName: KIND_NAMES[ITEMS[id]?.kind] ?? '' }; });
  const quests = (s.qlog ?? []).filter((q) => q.state === 'active' || q.state === 'ready').map((q) => {
    const def = QUESTS[q.questId];
    const objectives = def ? def.objectives.map((o, i) => ({ label: o.type === 'kill' ? ('Kill ' + mobName(o.targetMobId)) : ('Collect ' + displayItemName(o.itemId)), have: Math.min(q.counts[i] ?? 0, o.count), need: o.count })) : [{ label: 'Progress', have: q.state === 'ready' ? 1 : 0, need: 1 }];
    return { name: questName(q.questId), state: q.state, objectives };
  });
  const t = world.target();
  const target = (t && !t.dead && t.k === 'mob') ? { name: mobName(t.tid, t.nm), level: t.lv, hpPct: t.hp / Math.max(1, t.mhp) } : null;
  const auras = s.auras ?? [];
  ...
  const talents = talAlloc ? {
    spec: SPEC_NAMES[talAlloc.spec] ?? talAlloc.spec ?? '—',
    ...
    nodes: Object.entries(talAlloc.ranks ?? {}).map(([id, r]) => ({
      name: id === 'feral_choice' ? (TALENT_CHOICE_NAMES[talAlloc.choices?.[id]] ?? TALENT_INFO[id]?.label ?? id) : (TALENT_INFO[id]?.label ?? id),
      rank: r, max: TALENT_INFO[id]?.max ?? r,
    })),
  } : null;
  return { ...base, name: s.nm ?? name, cls, level: s.lv, hp: s.hp, mhp: s.mhp, mana: s.res, mres: s.mres, mana_enabled: (s.mres ?? 0) > 0, resName, xp: s.xp, xpNext: xpToNext(s.lv ?? 1), copper: s.copper, gold: splitCoin(s.copper), pos: { x: s.x, z: s.z }, stats: statsArr, equip, inv, quests, target, form, buffs, talents, tactical: tacticalView(world) };
}
...
// Apply a validated control message to a settings object in place. Returns an English log label on success,
