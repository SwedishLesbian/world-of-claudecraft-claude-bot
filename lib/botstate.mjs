...
export function richState({ world, action, zone, session, settings, logBuf, online, host, name, cls }) {
  ...
  const statsArr = [
    ...
    { k: 'Сила атаки', v: s.ap }, { k: 'Крит', v: s.crit != null ? (s.crit * 100).toFixed(1) + '%' : '—' },
  ].filter((x) => x.v != null);
  const SLOT_RU = { mainhand: 'Оружие', chest: 'Грудь', legs: 'Ноги', feet: 'Ступни' };
  ...
  const quests = (s.qlog ?? []).filter((q) => q.state === 'active' || q.state === 'ready').map((q) => {
    ...
    const objectives = def ? def.objectives.map((o, i) => ({ label: o.type === 'kill' ? ('Убить ' + ruMob(o.targetMobId)) : ('Собрать ' + ruItem(o.itemId)), have: Math.min(q.counts[i] ?? 0, o.count), need: o.count })) : [{ label: 'Прогресс', have: q.state === 'ready' ? 1 : 0, need: 1 }];
  ...
  const auras = s.auras ?? [];
  const form = auras.some((a) => a.kind === 'form_bear') ? 'Медведь 🐻' : auras.some((a) => a.kind === 'form_cat') ? 'Кот 🐱' : '—';
  const buffs = auras.filter((a) => a.kind !== 'form_bear' && a.kind !== 'form_cat').map((a) => a.name).slice(0, 8);
  const resName = { mana: 'мана', rage: 'ярость', energy: 'энергия' }[s.rtype] ?? 'ресурс';
...
export function applySetting(settings, m) {
  ...
  const onoff = m.value ? 'вкл' : 'выкл';
  const lbl = { paused: m.value ? 'Пауза включена' : 'Пауза снята', mode: `Режим: ${m.value}`, lootCorpses: `Подбор лута: ${onoff}`, buyFood: `Покупка еды: ${onoff}`, helpOthers: `Помощь игрокам: ${onoff}`, autoEquip: `Авто-экип: ${onoff}`, bearForm: `Медвежья форма: ${onoff}`, sellJunk: `Продажа хлама: ${onoff}`, levelCap: `Лимит уровня: ${settings.levelCap}` };
