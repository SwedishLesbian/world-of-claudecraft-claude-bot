...
export function decide(ctx) {
  ...
  // 2. FLEE — OVERWHELMED by DIFFICULTY (weighted aggroLoad exceeds capacity — same metric as the pull-accept,
  //    so we never accept a fight we'd then flee: a swarm of greys is brawled, three at-level mobs aren't) while
  //    hurt, OR a MULTI-mob brawl (aggro >= 2) gone critical (hp < CRIT_HP) when the heal can't keep up (OOM /
