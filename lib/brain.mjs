...
function rest(ctx) {
  ...
  ctx.input({});
  // restock when actually OUT of what we need: food for HP, drink for mana (was drink-only)
  if (!used && (self.copper ?? 0) >= 200 && ctx.settings.buyFood !== false &&
      ((needHp && !invHas(self, FOOD)) || (manaRes && mana < 0.85 && !invHas(self, DRINK)))) ctx.needRestock = true;
...
export function decide(ctx) {
  ...
  // 1. self-heal in combat at <=50% — but ONLY in a winnable fight (≤1 attacker). With a PACK (2+) on us we
  //    FLEE (step 2) with INSTANT-only survival instead: a cast-time heal here would just be cancelled by the
  //    run, so we'd neither heal nor escape (the old "run⇄cast-heal⇄run" stutter that got the bot killed).
  if (inCombat && hp < 0.5 && aggro.length < 2) {
  ...
  // 2. FLEE — only when genuinely OVERWHELMED (more attackers than we can brawl, aggro > combatCap) AND
  //    hurt. We FIGHT packs up to our capacity (a paladin/bear brawls a 2-pull and kills it; death is cheap,
  //    so a fled winnable pull is just lost XP). When truly swarmed, peel past the ~45yd leash so they evade.
  if (inCombat && aggro.length > cap && hp < T.fleeHp) ctx.fleeing = true;
  if (ctx.fleeing) {
    // Release the run once the pack is back DOWN to brawlable size (≤ cap) — "flee until they give up, THEN
    // turn and kill what's left". fleeSurvival layers INSTANT heals/shields/potions while we run (they don't
    // stop the run), so fleeing-and-healing beats turning to die, and the 45yd mob leash ends the chase.
    if (aggro.length <= cap) { ctx.fleeing = false; ctx.nav.stuck = 0; }
    else {
  ...
  if (T.quest) {
    ...
    if (qa) {
      ...
      if (qa.kind === 'kill') {
        ...
        // Give up FAST on a death-trap: defer after 3 deaths with NO kill (a camp we just can't do yet), OR
        // after a 2min no-kill stall (too dense to even pull). Either way grind up, then retry in 3min.
        else if ((deaths - c.deathsAt >= 3 || ctx.now() - c.since > 120000) && !(ctx.deferredQuests?.get(qa.quest) > ctx.now())) {
          const trap = deaths - c.deathsAt >= 3;
          ...
          // a DEATH-TRAP (not just dense): commit to grinding clean kills for 2min to OUT-LEVEL, instead of
          // bouncing straight into the next equally-hard quest camp (the lvl4 zone has several dense camps —
          // bandits/murlocs — so without this the bot thrashes between them, dying, and never gains a level).
          if (trap) ctx.grindUntil = ctx.now() + 120000;
