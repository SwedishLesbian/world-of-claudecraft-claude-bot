...
export function decide(ctx) {
  ...
  // 2. FLEE — OVERWHELMED (more attackers than we can brawl, aggro > cap) while hurt, OR a MULTI-mob brawl
  //    (aggro >= 2) that's gone critical (hp < CRIT_HP) and the heal can't keep up (OOM / cooldown / none —
  //    the step-1 heal ran first and didn't return, so it failed). We NEVER flee a LONE mob: one attacker is
  //    always winnable (stand, heal, or just auto-attack it — fleeing only bares our back to a faster chaser).
  if (inCombat && hp < T.fleeHp && (aggro.length > cap || (aggro.length >= 2 && hp < CRIT_HP))) ctx.fleeing = true;
  if (ctx.fleeing) {
    // Stop fleeing the moment the pack is down to a LONE mob (≤1 → turn and fight/heal it; chasing-and-fleeing
    // a single mob just gets us chipped from behind), OR it's brawlable AND we've recovered to RECOVER_HP. The
    // old flat "aggro<=cap" release cancelled a low-HP flee instantly; the lone-mob clause is what stops the bot
    // fleeing the last straggler into the ground. fleeSurvival layers INSTANT heals/shields/potions while we run.
    if (aggro.length <= 1 || (aggro.length <= cap && hp >= RECOVER_HP)) { ctx.fleeing = false; ctx.nav.stuck = 0; }
    else {
