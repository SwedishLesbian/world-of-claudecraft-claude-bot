...
export function decide(ctx) {
  ...
  const mob = stickCurrent((m) => mobMaxLevel(m.tid) === 0 || mobMaxLevel(m.tid) >= defendFloor) ?? w.nearestSafeMob(self.lv, combatCap(ctx.CLASS));   // finish the current mob, else pick the cheapest winnable instance (engageCost ≤ capacity)
