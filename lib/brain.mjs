    ...
    if (qa) {
      ...
      if (qa.kind === 'accept' || qa.kind === 'turnin') {
        ...
        if (dnow - g.since > 8000) {
          ...
          ctx.log('⏭ Quest "' + ruQuest(qa.quest) + '" could not be ' + (qa.kind === 'accept' ? 'accepted' : 'turned in') + '; skipping for 10 minutes');
          // fall through (no return) so the bot does something useful this tick instead of freezing
        } else {
          ctx.cmd({ cmd: qa.kind, quest: qa.quest });
          act(ctx, (qa.kind === 'accept' ? '📜 Accepting quest: ' : '✅ Turning in quest: ') + ruQuest(qa.quest));
          ctx.log((qa.kind === 'accept' ? 'Accepted quest: ' : 'Turning in quest: ') + ruQuest(qa.quest));
          return;
        }
      }
      if (qa.kind === 'goto') { act(ctx, '🧭 Going to NPC (' + ruQuest(qa.quest) + ')'); travelTo(ctx, qa.goal); return; }
      if (qa.kind === 'kill') {
        ...
        if (!atCamp) { act(ctx, '🧭 Going to camp (' + ruQuest(qa.quest) + ')'); travelTo(ctx, qa.goal); return; }
        ...
        if ((ctx.tooHard?.get(qa.quest)) !== self.lv) ctx.log('⏭ "' + ruQuest(qa.quest) + '" is too difficult; leveling before returning');
      ...
      if (qa.kind === 'collect_ground') {
        ...
        if (!c || c.qid !== qa.quest) ctx.collect = { qid: qa.quest, got, since: ctx.now(), pt: 0, deferUntil: 0 };
          ...
          ctx.log('⏭ Postponing collection for "' + ruQuest(qa.quest) + '" (no progress); trying other quests');
        }
        if (!(ctx.collect.deferUntil > ctx.now())) {
          ...
          if (objo) {
            if (w.dist(objo) <= 4.5) { ctx.cmd({ cmd: 'pickup', id: objo.id }); act(ctx, '📦 Picking up item (' + ruQuest(qa.quest) + ')'); return; }
            act(ctx, '🧭 Approaching item (' + ruQuest(qa.quest) + ')'); travelTo(ctx, { x: objo.x, z: objo.z }); return;
          }
          ...
          act(ctx, '🧭 Going to quest item (' + ruQuest(qa.quest) + ')'); travelTo(ctx, g); return;
        }
