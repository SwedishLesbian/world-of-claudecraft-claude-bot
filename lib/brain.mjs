...
import { mobName, questName } from './english.mjs';
import { ITEMS } from './items.generated.mjs';
import { VENDORS } from './vendors.generated.mjs';
...
export function fleeSurvival(ctx, aggro) {
  ...
  if (S.root && learned(self, S.root) && manaFrac > 0.15) {
    ...
    if (canCast(self, S.root)) {
      if (S.root.aoe) { ctx.cmd({ cmd: 'cast', ability: S.root.id }); for (const m of aggro) ctx.rootUntil.set('root:' + m.id, ROOT_UNTIL(now)); act(ctx, '🌀 Controlling the pack'); return false; }
      ...
      if (chaser) {
        ...
        ctx.cmd({ cmd: 'cast', ability: S.root.id }); ctx.rootUntil.set('root:' + chaser.id, ROOT_UNTIL(now)); act(ctx, '🌿 Rooting while fleeing');
  ...
  if (S.escape && learned(self, S.escape) && !hasAura(self, 'buff_speed')) {
    ...
    if (canCast(self, S.escape)) { ctx.cmd({ cmd: 'cast', ability: S.escape.id }); act(ctx, '💨 Speeding up to escape'); return S.escape.cast > 0; }
  }
...
function engage(ctx, mob, label) {
  const w = ctx.world, self = w.self, nm = mobName(mob.tid, mob.nm), K = ctx.kit, d = w.dist(mob);
  if (d > ctx.range) {
    ...
    if (ctx.CLASS === 'druid' && isBear(self) && opener && mob.aggro !== self.id && d <= PULL_RANGE && w.joinCount(mob, self.lv) > 0) {
      ...
      act(ctx, (label || 'Combat') + ': positioning to pull " ' + nm + ' " from the pack');
      return;
    }
    ...
    if (canRanged && (crowd > 0 || ctx.range > 5)) {
      ...
      act(ctx, (label || 'Combat') + (crowd > 0 ? ': pulling at range " ' + nm + ' " (nearby ' + crowd + ')' : ': attacking at range " ' + nm + ' "'));
      return;
    }
    ...
    act(ctx, (label || 'Combat') + ': approaching " ' + nm + ' "'); goTo(ctx, mob); return;
  }
  act(ctx, (label || 'Combat') + ': " ' + nm + ' " (' + Math.round((mob.hp / Math.max(1, mob.mhp)) * 100) + '%)');
  ctx.input({}, w.faceTo(mob));
  if (self.target !== mob.id) ctx.cmd({ cmd: 'target', id: mob.id });
...
export function paladinRotate(ctx, mob) {
  ...
  // 7. Judgement: instant Holy burst on cd. It CONSUMES the Seal, forcing a ~35-mana reapply next tick (step
  //    4) — that reseal is the paladin's main grind-time mana drain. CONSERVE on a LONE mob at low mana: skip
  //    the Judgement→reseal loop (the standing Seal + auto-attacks already kill a trivial target) and bank the
  //    mana for an emergency heal instead of OOMing into a long rest. On a pack (meleeOnMe>=2) or with mana to
  //    spare, judge normally for the burst — exactly when the extra damage matters.
  const judge = (K.nukes ?? []).find((n) => n.id === 'judgement');
  if (judge && hasSeal && canCast(self, judge) && !(manaFrac < 0.30 && meleeOnMe < 2)) { ctx.cmd({ cmd: 'cast', ability: judge.id }); return; }
  ...
  if (K.healOthers) {
    ...
    if (hurt && ctx.helpLog.length < 6) {
      ...
      if (ctx.CLASS === 'druid' && isBear(self)) {
        if (hurt.frac < 0.35) { ctx.cmd({ cmd: 'cast', ability: 'bear_form' }); act(ctx, '🐻➡ Leaving form to heal'); return true; }
      } else {
        const ab = (hurt.frac >= 0.4 && K.healOthersHot) ? K.healOthersHot : K.healOthers;
        if (canCast(self, ab)) { ctx.cmd({ cmd: 'target', id: hurt.id }); ctx.cmd({ cmd: 'cast', ability: ab.id }); ctx.healThrottle.set(hurt.id, now); ctx.helpLog.push(now); act(ctx, '💚 Healing player ' + (hurt.nm || '')); ctx.log('Healing player ' + (hurt.nm || '#' + hurt.id)); return true; }
      }
    }
  }
  ...
  if (!(ctx.CLASS === 'druid' && isBear(self))) {
    for (const b of (K.buffOthers ?? [])) {
      ...
      for (const c of cands) {
        ...
        ctx.cmd({ cmd: 'target', id: c.id }); ctx.cmd({ cmd: 'cast', ability: b.id }); ctx.buffThrottle.set(key, now); act(ctx, '✨ Buffing ' + (c.nm || '')); return true;
      }
    }
  }
  return false;
...
function manageTalents(ctx) {
  ...
  act(ctx, `🌳 Talents: ${desired.spec} (${spent}/${budget} points)`);
  return true;
}
...
function buffSelfUpkeep(ctx) {
  ...
  for (const b of list) {
    ...
    ctx.cmd({ cmd: 'cast', ability: b.id }); ctx.buffSelfThrottle.set(b.id, now); act(ctx, '✨ Buff: ' + b.id); return true;
  }
  return false;
}
...
function autoEquip(ctx) {
  ...
  for (const it of (self.inv ?? [])) {
    ...
    if (gearScore(meta, ROLE) > gearScore(ITEMS[curId], ROLE)) {                   // role-aware: survival-first for armor
      ctx.triedEquip.add(id); ctx.cmd({ cmd: 'equip', item: id }); act(ctx, '🛡 Equipping ' + (meta.name || id)); ctx.log('Equipped: ' + (meta.name || id)); return true;
    }
  }
  return false;
}

// ---- vendor gear: BUY upgrades, DATA-DRIVEN from the game's vendorItems ------------------------------
// Source of truth is vendors.generated.mjs (projected from NPCS.vendorItems) — no hardcoded stock lists,
// so a content patch (new vendor, restocked slot) is picked up by `node scripts/gen_bot_vendors.mjs` with
// zero code change. Filling an empty / under-quality slot is the single biggest survival win for an
// under-geared bot. Some gear vendors are NOT the zone food vendor (zone1 smith_haldren has the only L3-7
// starter armor+weapons; zone3 armorer_hode has the L15-18 weapons), so the bot routes to them directly
// (decide() picks the nearest gear vendor with an affordable upgrade). autoEquip equips the buy next tick.
const buyPrice = (id) => ITEMS[id]?.buyValue ?? 0;
// equippable gear (weapon/armor with a slot AND a buy price) a vendor stocks, straight from game data.
export const vendorGear = (tid) => (VENDORS[tid]?.items ?? []).filter((id) => { const m = ITEMS[id]; return m && (m.kind === 'weapon' || m.kind === 'armor') && m.slot && m.buyValue != null; });
// every vendor that sells equippable gear, as routable {tid,x,z}; gearVendorsNear sorts nearest-first.
const GEAR_VENDORS = Object.entries(VENDORS).filter(([tid]) => vendorGear(tid).length).map(([tid, v]) => ({ tid, x: v.pos.x, z: v.pos.z }));
export const gearVendorsNear = (pos) => GEAR_VENDORS.slice().sort((a, b) => dist2(pos, a) - dist2(pos, b));
// a usable, affordable, in-stock item we don't already own that out-scores what's in its slot (role-aware).
function isGearUpgrade(ctx, id, role, owned) {
  const meta = ITEMS[id], self = ctx.world.self;
  if (!meta || !meta.slot) return false;
  if (meta.requiredClass && !meta.requiredClass.includes(ctx.CLASS)) return false;   // can't use it (wrong class)
  if (owned?.has(id)) return false;                                                  // already bought, awaiting equip
  if ((self.copper ?? 0) < buyPrice(id)) return false;                               // can't afford it yet
  return gearScore(meta, role) > gearScore(ITEMS[self.equip?.[meta.slot]], role);    // empty slot or a real upgrade
}
// true if THIS vendor sells an affordable upgrade NOW — only then is a gear-trip to it worth making.
export function needsVendorGear(ctx, tid) {
  const role = gearRole(ctx), owned = new Set((ctx.world.self.inv ?? []).map((it) => it.itemId ?? it.id));
  for (const id of vendorGear(tid)) if (isGearUpgrade(ctx, id, role, owned)) return true;
  return false;
}
// buy ONE piece per tick at vendor `tid`: the best-scoring affordable upgrade PER SLOT (so we don't waste
// coin on an interim chest when the better one is already affordable too).
export function buyGear(ctx, tid) {
  const w = ctx.world, self = w.self;
  const npc = [...w.ents.values()].find((e) => e.k === 'npc' && e.tid === tid);
  if (!npc) return false;
  const owned = new Set((self.inv ?? []).map((it) => it.itemId ?? it.id));
  const now = ctx.now(), ROLE = gearRole(ctx), bySlot = new Map();
  for (const id of vendorGear(tid)) {
    if ((ctx.gearBuyAt?.get(id) ?? 0) > now - 8000) continue;                        // just tried (avoid double-buy before it lands in inv)
    if (!isGearUpgrade(ctx, id, ROLE, owned)) continue;
    const meta = ITEMS[id], sc = gearScore(meta, ROLE), prev = bySlot.get(meta.slot);
    if (!prev || sc > prev.sc) bySlot.set(meta.slot, { id, meta, sc });             // keep only the best per slot
  }
  for (const { id, meta } of bySlot.values()) {
    (ctx.gearBuyAt ??= new Map()).set(id, now);
    ctx.cmd({ cmd: 'buy', npc: npc.id, item: id });
    act(ctx, '🛒 Buying equipment: ' + (meta.name || id)); ctx.log('Bought equipment: ' + (meta.name || id));
    return true;                                                                     // one buy per tick
  }
  return false;
}
...
function sellJunk(ctx) {
  ...
  act(ctx, '💰 Selling ' + (s.meta.name || s.id));
  ctx.log('Sold: ' + (s.meta.name || s.id) + (val ? ' (+' + val + 'c)' : ''));
  return true;
}

// ---- rest ---------------------------------------------------------------
// Only trader_wilkes (zone1) stocks minor_healing_potion (game content/zone1.ts vendorItems — no zone2/3 food
// vendor does), so a heal-pot restock TRIP only makes sense when the nearest food vendor is one that sells them.
// Otherwise the bot walked to a zone2/3 vendor that silently drops the buy, never got the pots (!invHas stays
// true), and re-tripped forever. It stocks a BUFFER at zone1 and carries it out; in zone2/3 it just goes without.
const HEAL_POT_VENDORS = new Set(['trader_wilkes']);
function rest(ctx) {
  const w = ctx.world, self = w.self;
  ...
  // PALADIN melees freely (auto-attack is free) so it fights at lower mana than a pure caster — but its
  // SURVIVAL comes from mana-funded heals, so it must NOT enter a fight dry. The old 6% floor (shared with
  // bear) left an under-levelled paladin healless in a pack: it flees at ~10% HP before the kill, the mob
  // regens to full, repeat → 0 kills. So it keeps a heal cushion (rest mana to ~45% before pulling, like a
  // caster). Only the BEAR druid still fights truly mana-free — it's in form (mana FROZEN) and uses no
  // mana heals — so it alone rests mana just near-OOM (< 6%, enough to afford the shift + a clutch HoT).
  const needHp = hp < 0.6, needMana = manaRes && mana < (canBearFight ? 0.06 : 0.45);
  ...
  if (needHp) {
    ...
    if (heal && !DRAIN_HEALS.has(heal.id) && canCast(self, heal)) {
      ...
      act(ctx, '💚 Healing while resting'); ctx.input({}); return true;
    }
  }
  ...
  if (needMana && ctx.now() > ctx.potionCdUntil) {
    ...
    if (mp) { ctx.cmd({ cmd: 'use', item: mp }); ctx.potionCdUntil = ctx.now() + 60000; act(ctx, '🧪 Drinking a mana potion'); ctx.input({}); return true; }
  }
  if (self.eat || self.drk) { act(ctx, '🍗 Resting (eating/drinking)'); ctx.input({}); return true; }
  let used = false;
  ...
  act(ctx, used ? '🧘 Resting and recovering' : '🧘 Waiting and recovering');
  ctx.input({});
  ...
  // heal-pot restock only when a vendor that STOCKS them is the nearest food vendor (else a wasted zone2/3 trip).
  const healPotRestock = !invHas(self, HEAL_POT) && HEAL_POT_VENDORS.has(foodVendorNear(w.pos()).tid);
  if (!used && (self.copper ?? 0) >= 200 && ctx.settings.buyFood !== false &&
      ((needHp && !invHas(self, FOOD)) || (manaRes && mana < 0.85 && !invHas(self, DRINK)) || healPotRestock)) ctx.needRestock = true;
  // keep resting until topped: bear druid only to ~20% mana (fights in form), PALADIN to ~55% (a real heal
  // cushion so it can sustain a pack and finish the kill, not flee dry), pure casters to ~70%.
  const manaKeep = canBearFight ? 0.2 : (ctx.CLASS === 'paladin' ? 0.55 : 0.7);
  return hp < 0.9 || (manaRes && mana < manaKeep);
}
...
function restock(ctx) {
  ...
  ctx.log('Restocking at vendor (' + vendor.tid + ')');
}
...
export function decide(ctx) {
  ...
  // Tell the world OUR combat profile so threatWeight knows which v0.10.0 mob affixes can actually bite us:
  // caster = spell-reliant (a silence/spell-reflect/mana-drain cripples a druid root-kite or pure caster;
  // mirrors the gear-role test), melee = weapon-reliant (a disarm/blind matters). A hybrid is both. Class is
  // fixed per process — set once, not every tick. (Guarded: a partial-mock world in tests has no
  // setCombatProfile — skip, like the null-safe ??= maps above, so this never throws before the survival
  // decision that runs further down.)
  if (w.setCombatProfile && w._profileClass !== ctx.CLASS) {
    w.setCombatProfile({
      caster: !ctx.kit?.melee || ctx.CLASS === 'druid' || ctx.CLASS === 'paladin' || ctx.CLASS === 'shaman',
      melee: !!ctx.kit?.melee,
    });
    w._profileClass = ctx.CLASS;
  }
  ...
  if (S.paused) { if (self.dead) ctx.cmd({ cmd: 'release' }); else ctx.input({}); act(ctx, '⏸ Paused'); return; }
  if (self.dead) { ctx.cmd({ cmd: 'release' }); ctx.nav.stuck = 0; ctx.nav.anchorX = undefined; act(ctx, '💀 Resurrecting at the graveyard'); return; }
  ...
  if (self.x > 220 || self.x < -220 || self.z < -240 || self.z > 960) {
    ...
    if (exit && w.dist(exit) <= 5) { ctx.cmd({ cmd: 'leave_dungeon' }); ctx.cmd({ cmd: 'leave_crypt' }); act(ctx, '🚪 Leaving the dungeon'); return; }
    if (w.dist(goal) <= 5) { ctx.cmd({ cmd: 'leave_dungeon' }); ctx.cmd({ cmd: 'leave_crypt' }); act(ctx, '🚪 Leaving the dungeon'); return; }
    act(ctx, '🚪 Going to the dungeon exit'); goTo(ctx, goal); return;
  }
  ...
  for (const d of DUNGEON_DOORS) {
    ...
    if (dist2(self, d) < (nodeNearDoor ? 2.5 : 5)) { ctx.input({ f: 1 }, w.faceTo(d) + Math.PI); ctx.nav.stuck = 0; act(ctx, '🚪 Staying away from the dungeon entrance'); return; }
  }
  ...
  if (inCombat && !ctx.fleeing && aggro.length <= cap && hp < 0.5) {
    if (ctx.CLASS !== 'druid' && trySelfHeal(ctx, 0.5, nearest(aggro, w))) { act(ctx, '🩹 Healing'); return; }
    if (hp < 0.35 && invHas(self, HEAL_POT) && ctx.now() > ctx.potionCdUntil) { ctx.cmd({ cmd: 'use', item: invFind(self, HEAL_POT) }); ctx.potionCdUntil = ctx.now() + 60000; act(ctx, '🧪 Drinking a potion'); return; }
  }
  // 2. FLEE — OVERWHELMED by DIFFICULTY (weighted aggroLoad exceeds capacity — the SAME metric as the pull-accept
  //    gate, so we don't pull what we'd instantly flee: a swarm of greys is brawled, three at-level mobs aren't)
  //    AND hurt (hp < fleeHp — a healthy bot still TANKS an over-cap load; death is cheap, fleeing at full HP just
  //    wastes the fight), OR a MULTI-mob brawl (aggro >= 2) gone critical (hp < CRIT_HP) when the heal can't (OOM /
  //    cooldown / none — step-1 heal ran first and didn't return, so it failed). We NEVER flee a LONE mob: one
  //    attacker is always winnable (stand, heal, or just auto-attack it — fleeing only bares our back to a chaser).
  const load = w.aggroLoad(self.lv);
  if (inCombat && hp < T.fleeHp && (load > cap || (aggro.length >= 2 && hp < CRIT_HP))) ctx.fleeing = true;
  if (ctx.fleeing) {
    // Stop fleeing the moment the pack is down to a LONE mob (≤1 → turn and fight/heal it; chasing-and-fleeing
    // a single mob just gets us chipped from behind), OR the weighted load is back within capacity AND we've
    // recovered to RECOVER_HP. The lone-mob clause stops the bot fleeing the last straggler into the ground.
    // fleeSurvival layers INSTANT heals/shields/potions while we run.
    if (aggro.length <= 1 || (load <= cap && hp >= RECOVER_HP)) { ctx.fleeing = false; ctx.nav.stuck = 0; }
    else {
      ...
      ctx.input({ f: 1 }, w.faceTo(centroid(aggro)) + Math.PI); act(ctx, '🏃 Escaping (' + aggro.length + ')'); return;
    }
  }
  ...
  if (!inCombat && (hp < 0.6 || lowMana) && w.hostilesNear(18).length > 0) { fleeFrom(ctx, nearest(w.hostilesNear(18), w)); act(ctx, '🚶 Moving away to rest'); return; }
  ...
  if (!inCombat && hp < T.readyHp && manaResOK && trySelfHeal(ctx, T.readyHp, null)) { act(ctx, '🩹 Topping up health'); return; }
  ...
  if (S.lootCorpses !== false) {
    ...
    if (corpse) { if (w.dist(corpse) > 4) { act(ctx, '👜 Moving to loot'); goTo(ctx, corpse); } else { act(ctx, '👜 Looting'); ctx.cmd({ cmd: 'loot', id: corpse.id }); } return; }
  }
  // 5a. equip upgrades
  if (S.autoEquip && !inCombat && autoEquip(ctx)) return;
  ...
  if (!inCombat) {
    ...
    const v = foodVendorNear(w.pos()), atFood = w.dist(v) <= 6;
    // GEAR: a dedicated trip to BUY an upgrade (fill an empty slot / beat what's equipped) once affordable —
    // the single biggest survival win for an under-geared bot. Target the NEAREST gear vendor with an
    // affordable upgrade now; this can be a NON-food vendor (zone1 smith_haldren, zone3 armorer_hode).
    // Per-vendor cooldown so a no-op pass at one vendor doesn't lock out a different one that does stock it.
    // shop LOCALLY: only vendors in the bot's current zone are candidates, so it never treks cross-zone
    // for gear it could buy after it naturally progresses there (matches the old foodVendorNear behaviour).
    const myZone = zoneIdxAtZ(w.pos().z);
    const gv = gearVendorsNear(w.pos()).find((g) => {
      if (zoneIdxAtZ(g.z) !== myZone) return false;
      const cdOk = ctx.gearTripAt?.tid !== g.tid || (ctx.now() - (ctx.gearTripAt?.at ?? -1e9)) > T.gearTrip;
      return cdOk && needsVendorGear(ctx, g.tid);
    });
    ctx.needGear = !!gv;
    if (atFood && S.sellJunk !== false && sellJunk(ctx)) return;          // sell one stack per tick at the food vendor
    if (gv) {                                                             // route to the gear vendor, buy one piece per tick
      if (w.dist(gv) > 6) { act(ctx, '🛒 Going to buy equipment'); travelTo(ctx, gv); return; }
      if (buyGear(ctx, gv.tid)) return;
      ctx.gearTripAt = { tid: gv.tid, at: ctx.now() }; ctx.needGear = false; // nothing landed this tick -> cooldown THIS vendor
    }
    if ((ctx.needVendor && S.sellJunk !== false) || (ctx.needRestock && S.buyFood !== false)) {
      if (!atFood) { act(ctx, '🛒 Going to the vendor'); travelTo(ctx, v); return; }
      ctx.needVendor = false;
      if (ctx.needRestock && S.buyFood !== false) { act(ctx, '🛒 Restocking'); restock(ctx); ctx.needRestock = false; }
      return;
    }
  }

  // level cap reached -> stop questing/grinding (still survives + helps)
  if ((self.lv ?? 1) >= (S.levelCap ?? 20)) { act(ctx, '🏁 Reached level cap ' + (S.levelCap ?? 20)); ctx.input({}); return; }

  // passive mode: defend only (fight everything that's on us, but commit to one target — no thrash)
  if (S.mode === 'passive') {
    if (inCombat) { engage(ctx, commit(aggro), 'Defending'); return; }
    act(ctx, '🛡 Passive mode'); ctx.input({}); return;
  }

  // 6/7/8. quests (modes with tune.quest: quest, level-fast, cautious — grind/farm-gold/passive skip).
  // ctx.outLevel (set by the watchdog on a death-loop) SKIPS quests: a quest camp above our weight is what
  // got us killed, so we fall through to grind the safest camp (patrol picks easiest-first) and out-level first.
  if (T.quest && !ctx.outLevel) {
    // SKIP set for objective selection, from two live sources (so the bot rotates to a doable objective instead
    // of idling on a stuck one): (1) deferredQuests — barren/guarded collect spots + the accept/turn-in anti-
    // stuck guard, time-based; (2) tooHard — a kill camp whose every instance currently costs more than our
    // capacity, keyed to LEVEL (not a timer): it self-clears the instant we level up, so we retry stronger.
    ...
    if (ctx.tooHard && ctx.tooHard.size) {
      for (const [q, lv] of ctx.tooHard) { if (lv !== self.lv) ctx.tooHard.delete(q); else (deferSet ??= new Set()).add(q); }
    }
    ...
    if (qa) {
      ...
      // looping 'Accepting quest: Bastion Gate'). If the SAME accept/turn-in is still being returned after
      ...
      if (qa.kind === 'accept' || qa.kind === 'turnin') {
        ...
        if (dnow - g.since > 8000) {
          ...
          ctx.log('⏭ Quest "' + questName(qa.quest) + '" could not be ' + (qa.kind === 'accept' ? 'accepted' : 'turned in') + '; skipping for 10 minutes');
          // fall through (no return) so the bot does something useful this tick instead of freezing
        } else {
          ctx.cmd({ cmd: qa.kind, quest: qa.quest });
          act(ctx, (qa.kind === 'accept' ? '📜 Accepting quest: ' : '✅ Turning in quest: ') + questName(qa.quest));
          ctx.log((qa.kind === 'accept' ? 'Accepted quest: ' : 'Turning in quest: ') + questName(qa.quest));
          return;
        }
      }
      if (qa.kind === 'goto') { act(ctx, '🧭 Going to NPC (' + questName(qa.quest) + ')'); travelTo(ctx, qa.goal); return; }
      if (qa.kind === 'kill') {
        // UNIVERSAL kill decision: KILL the cheapest WINNABLE instance, else PUSH toward the camp, else RETREAT
        // to winnable content (fall through to the grinder). Winnability is one live test — engageCost ≤ capacity
        // (world.mjs) — recomputed every tick, so there are NO defer timers; a too-hard camp marks itself "not
        // yet" AT THIS LEVEL and resumes the moment we level up (the entry no longer matches self.lv).
        const cap = combatCap(ctx.CLASS);
        // KILL: stick to the mob we're already fighting until it dies (re-picking every tick finishes none),
        // else the cheapest winnable instance.
        const qm = stickCurrent((m) => m.tid === qa.mobId) ?? w.questMob(qa.mobId, self.lv, cap);
        if (qm) { engage(ctx, qm, '⚔ Quest'); return; }
        // KILL: something is already beating on us -> finish that fight first.
        { const d = defendTarget(); if (d) { engage(ctx, d, '⚔ Fighting back'); return; } }
        // KILL on the way: a winnable, level-appropriate mob already in reach -> grind it as we close in.
        const inPath = w.nearestSafeMob(self.lv, cap);
        if (inPath && w.dist(inPath) <= ctx.range + 1) { engage(ctx, inPath, '⚔ Along the way'); return; }
        // PUSH: the camp isn't in view yet -> travel to it (winnability is judged on arrival, not from afar).
        const atCamp = w.mobs().some((m) => m.tid === qa.mobId && w.dist(m) < 70);
        if (!atCamp) { act(ctx, '🧭 Going to camp (' + questName(qa.quest) + ')'); travelTo(ctx, qa.goal); return; }
        // RETREAT to winnable content: the camp is HERE but nothing in it is winnable yet (every instance's cost
        // exceeds capacity) and there's nothing level-appropriate to grind in reach. Standing here refusing is the
        // no-kill spiral; diving is a needless death. Mark this quest "too hard AT THIS LEVEL" so selection skips
        // it (no ping-pong back to the camp) and fall through to the grinder, which finds winnable mobs / routes
        // to a level-appropriate zone. The mark self-clears the instant we level up — retry stronger, no timers.
        if ((ctx.tooHard?.get(qa.quest)) !== self.lv) ctx.log('⏭ "' + questName(qa.quest) + '" is too difficult; leveling before returning');
        (ctx.tooHard ??= new Map()).set(qa.quest, self.lv);
        // fall through (no return) → global grinder / patrol
      }
      if (qa.kind === 'collect_ground') {
        ...
        if (!c || c.qid !== qa.quest) ctx.collect = { qid: qa.quest, got, since: ctx.now(), pt: 0, deferUntil: 0 };
          ...
          ctx.log('⏭ Postponing collection for "' + questName(qa.quest) + '" (no progress); trying other quests');
        }
        if (!(ctx.collect.deferUntil > ctx.now())) {
          ...
          if (objo) {
            if (w.dist(objo) <= 4.5) { ctx.cmd({ cmd: 'pickup', id: objo.id }); act(ctx, '📦 Picking up item (' + questName(qa.quest) + ')'); return; }
            act(ctx, '🧭 Approaching item (' + questName(qa.quest) + ')'); travelTo(ctx, { x: objo.x, z: objo.z }); return;
          }
          ...
          act(ctx, '🧭 Going to quest item (' + questName(qa.quest) + ')'); travelTo(ctx, g); return;
        }
  ...
  { const d = defendTarget(); if (d) { engage(ctx, d, 'Combat'); return; } }
  const stale = (ctx.now() - (ctx.lastKill ?? ctx.now())) > 70000;
  let mob = stickCurrent((m) => mobMaxLevel(m.tid) === 0 || mobMaxLevel(m.tid) >= defendFloor) ?? w.nearestSafeMob(self.lv, combatCap(ctx.CLASS));   // finish the current mob, else pick the cheapest winnable instance (engageCost ≤ capacity)
  // OUT-LEVEL retreat: don't PROACTIVELY pull an over-level mob (a "winnable" single fen_troll that links into
  // a 3-pull is what got us killed) — drop to patrol, which routes to the safest camp. Mobs already on us are
  // still fought above (defendTarget); this only refuses fresh over-level pulls so the retreat actually leaves.
  if (ctx.outLevel && mob && mobMaxLevel(mob.tid) > (self.lv ?? 1)) mob = null;
  if (mob) { engage(ctx, mob, 'Grinding'); return; }
...
function patrol(ctx, stale) {
  ...
  if (!camps.length) { act(ctx, '🧭 Going to a lower-level zone'); travelTo(ctx, ZONES[Math.max(0, maxZ - 1)].hub); return; }
  ...
  if (!nav.patrolGoal || reached || timedOut) {
    ...
    // OUT-LEVEL retreat (death-loop): COMMIT to the SAFEST camp — lowest mob level that still pays xp
    // (campsForLevel already floors at lv-3, so it's never grey) — and grind up there instead of dying on
    // an over-weight camp. This is the "easiest by maxLevel" routing, but gated to the death-loop case where
    // trekking to safety is exactly right (normal play stays nearest-first, no grey-grinding regression).
    const byNear = camps.map((c) => ({ c, d: dist2(here, c) })).sort((a, b) => a.d - b.d);
    if (ctx.outLevel) {
      const byEasy = camps.slice().sort((a, b) => (a.maxLevel - b.maxLevel) || (dist2(here, a) - dist2(here, b)));
      nav.patrolGoal = byEasy[0];
    } else {
  ...
  act(ctx, '🧭 ' + (ctx.outLevel ? 'Leveling on easier enemies' : (stale ? 'Looking for suitable enemies' : 'Going to enemies')) + (zn ? ' → ' + zn : ''));
