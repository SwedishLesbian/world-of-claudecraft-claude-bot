...
export function decide(ctx) {
  ...
  if (!inCombat) {
    ...
    if (gv) {                                                             // route to the gear vendor, buy one piece per tick
      if (w.dist(gv) > 6) { act(ctx, '🛒 Иду за экипировкой'); travelTo(ctx, gv); return; }
    ...
    if ((ctx.needVendor && S.sellJunk !== false) || (ctx.needRestock && S.buyFood !== false)) {
      if (!atFood) { act(ctx, '🛒 Иду к торговцу'); travelTo(ctx, v); return; }
  ...
  if (T.quest && !ctx.outLevel) {
    ...
    if (qa) {
      ...
      if (qa.kind === 'kill') {
        ...
        if (!atCamp) { act(ctx, '🧭 Иду к лагерю (' + ruQuest(qa.quest) + ')'); travelTo(ctx, qa.goal); return; }
        ...
        if ((ctx.tooHard?.get(qa.quest)) !== self.lv) ctx.log('⏭ «' + ruQuest(qa.quest) + '» пока не по зубам — качаюсь, вернусь сильнее');
...
function patrol(ctx, stale) {
  ...
  act(ctx, '🧭 ' + (ctx.outLevel ? 'Докачиваюсь на лёгких' : (stale ? 'Ищу мобов по силам' : 'Иду к мобам')) + (zn ? ' → ' + zn : ''));
