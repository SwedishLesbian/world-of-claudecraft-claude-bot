...
const TOKEN_DIR = new URL('../.tokens/', import.meta.url);   // Gitignored; holds authentication secrets.
...
export class Connection {
    ...
    if (needAuth || !this.token) {
      try { const a = await this.getAuth(this._needFreshToken); this._needFreshToken = false; this.token = a.token; this.charId = a.charId; }
        ...
        if (needsBrowserToken) {
          console.error('[net] ⛔ Login blocked by Cloudflare Turnstile (403); REST login cannot complete it.');
          console.error('[net]    Get a fresh browser token: node get-token.mjs');
          console.error('[net]    Waiting 3 minutes before trying the token cache again.');
        } else {
  ...
  _onClose() {
    ...
    if (this._authReject === 'suspended') {      // account suspended: back off until the unsuspend time (capped), NOT every 30s
      ...
      console.error(`[net] ACCOUNT SUSPENDED${this._suspendedUntil ? ' until ' + new Date(this._suspendedUntil).toISOString() : ''}; retrying in ${Math.round(wait / 60000)} minutes.`);
      ...
      this._authReject = null;
      console.log('[net] The previous server session is still closing; retrying with the same token in 1.5s.');
