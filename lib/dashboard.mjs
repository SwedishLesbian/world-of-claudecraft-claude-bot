...
export class Dashboard {
  ...
  start() {
    ...
    this.server.on('error', (e) => {
      if (e.code === 'EADDRINUSE') console.error(`[dashboard] порт ${this.port} занят — дашборд не запущен, но бот играет. Закрой старый процесс или задай DASH_PORT.`);
    ...
    try { this.server.listen(this.port, this.host, () => console.log(`[dashboard] открой http://localhost:${this.port}/  (bind ${this.host}; токен подставляется в URL автоматически)`)); } catch (e) { console.error('[dashboard]', e.message); }
  }
  broadcast(state) {
...
<div class="action" id="action">Подключение…</div>
<div class="grid">
  <div class="card c5">
    <h2>Персонаж</h2>
    ...
    <div class="row"><span class="k">Золото (всего)</span><span class="v gold" id="gold">—</span></div>
    <div class="row"><span class="k">Зона</span><span class="v" id="zone">—</span></div>
    <div class="row"><span class="k">Форма</span><span class="v" id="form">—</span></div>
    <div class="row"><span class="k">Бафы</span><span class="v" id="buffs" style="font-size:11px">—</span></div>
    <div class="row"><span class="k">Позиция</span><span class="v" id="pos">—</span></div>
    <div class="row"><span class="k">Цель</span><span class="v" id="tgt">нет</span></div>
  </div>
  <div class="card c3"><h2>Характеристики</h2><div id="stats"></div></div>
  <div class="card c3"><h2>Таланты</h2><div id="talents"><span class="empty">—</span></div></div>
  <div class="card c4"><h2>Сессия</h2><div id="session"></div></div>

  <div class="card c4"><h2>Экипировка</h2><table id="equip"></table></div>
  <div class="card c4"><h2>Настройки</h2><div id="settings"></div><button class="pause" id="pauseBtn">⏸ Пауза</button></div>
  <div class="card c4"><h2>Квесты</h2><div id="quests"><span class="empty">нет активных</span></div></div>

  <div class="card c5"><h2>Инвентарь / лут</h2><table id="inv"></table></div>
  <div class="card c7"><h2>Лог (live)</h2><div class="log" id="log"></div></div>
</div></div>
<script>
...
function connect(){
  ...
  ws.onclose=()=>{document.getElementById('conn').textContent='нет связи с ботом — переподключение…';setTimeout(connect,2000);};
}
...
function render(){
  ...
  document.getElementById('who').textContent=(S.cls?('· '+S.cls+' · ур. '+S.level):'');
  document.getElementById('conn').textContent=S.online?('онлайн · '+(S.server||'')):'оффлайн';
  document.getElementById('action').innerHTML='<b>Сейчас:</b> '+esc(S.action||'…');
  // bars
  ...
  if(S.xpNext){bar('xpb','опыт '+S.xp+' / '+S.xpNext,S.xp/S.xpNext);}
  ...
  document.getElementById('gold').textContent=S.gold?(S.gold.g+'з '+S.gold.s+'с '+S.gold.c+'м'):'—';
  document.getElementById('pos').textContent=S.pos?(Math.round(S.pos.x)+', '+Math.round(S.pos.z)):'—';
  document.getElementById('tgt').innerHTML=S.target?(esc(S.target.name)+' <span class="sub">ур.'+S.target.level+' · '+Math.round(S.target.hpPct*100)+'%</span>'):'нет';
  // stats
  ...
  // talents (v0.6)
  var tl=S.talents;
  document.getElementById('talents').innerHTML=tl?('<div class="row"><span class="k">'+esc(tl.spec)+'</span><span class="v gold">'+tl.spent+'/'+tl.total+' очк.</span></div>'+(tl.nodes||[]).map(function(n){return '<div class="row"><span class="k" style="font-size:11px">'+esc(n.name)+'</span><span class="v">'+n.rank+'/'+n.max+'</span></div>';}).join('')+(tl.spent<tl.total?'<div class="row"><span class="k sub">не распределено</span><span class="v gold">'+(tl.total-tl.spent)+'</span></div>':'')):'<span class="empty">нет (с 10 ур.)</span>';
  // session
  const ss=S.session||{};
  const gAll=S.gold?(S.gold.g+'з '+S.gold.s+'с '+S.gold.c+'м'):'—';
  document.getElementById('session').innerHTML=
    row('Время в сессии',fmtDur(ss.runtimeSec))+row('Убийств',ss.kills)+row('Смертей',ss.deaths)+
    row('Квестов сдано',ss.questsDone)+row('Опыта за сессию',ss.xpGained)+
    row('Золота за сессию',coin(ss.copperGained))+row('Золота всего (аккаунт)',gAll);
  // equipment
  document.getElementById('equip').innerHTML=(S.equip||[]).map(e=>'<tr><td class="k">'+esc(e.slotRu)+'</td><td style="color:'+(QC[e.quality]||'#fff')+'">'+esc(e.name||'—')+'</td></tr>').join('')||'<tr><td class="empty">пусто</td></tr>';
  // quests
  document.getElementById('quests').innerHTML=(S.quests&&S.quests.length)?S.quests.map(q=>'<div class="q"><div class="ttl">'+esc(q.name)+(q.state==='ready'?' ✅':'')+'</div>'+q.objectives.map(o=>'<div class="obj">'+esc(o.label)+': '+o.have+'/'+o.need+'</div><div class="objbar"><i style="width:'+Math.min(100,100*o.have/o.need)+'%"></i></div>').join('')+'</div>').join(''):'<span class="empty">нет активных</span>';
  // inventory
  document.getElementById('inv').innerHTML=(S.inv&&S.inv.length)?S.inv.map(i=>'<tr><td style="color:'+(QC[i.quality]||'#fff')+'">'+esc(i.name)+' <span class="sub">'+esc(i.kindRu||'')+'</span></td><td class="cnt">'+(i.count>1?('×'+i.count):'')+'</td></tr>').join(''):'<tr><td class="empty">пусто</td></tr>';
  ...
  if(setEl.dataset.sig!==sig && !setEl.contains(document.activeElement)){
    ...
    setEl.innerHTML=
      '<div class="set"><span>Режим</span><select id="mode">'+['quest','grind','level-fast','farm-gold','cautious','passive'].map(m=>'<option value="'+m+'"'+(st.mode===m?' selected':'')+'>'+({quest:'Квесты',grind:'Фарм мобов','level-fast':'Быстрый набор ур.','farm-gold':'Фарм золота',cautious:'Осторожный',passive:'Пассивный'}[m])+'</option>').join('')+'</select></div>'+
      toggle('lootCorpses',st.lootCorpses,'Подбирать лут')+
      toggle('buyFood',st.buyFood,'Покупать еду/воду')+
      toggle('helpOthers',st.helpOthers,'Помогать игрокам')+
      toggle('autoEquip',st.autoEquip,'Надевать апгрейды')+
      toggle('bearForm',st.bearForm,'Медвежья форма (друид 10+)')+
      toggle('sellJunk',st.sellJunk,'Продавать хлам/замены')+
      '<div class="set"><span>Качаться до уровня</span><input id="lvlcap" type="number" min="2" max="20" value="'+(st.levelCap??20)+'" style="width:64px;background:var(--panel2);color:var(--txt);border:1px solid var(--bd);border-radius:6px;padding:4px"></div>';
  ...
  const pb=document.getElementById('pauseBtn'); pb.className='pause'+(st.paused?' on':''); pb.textContent=st.paused?'▶ Возобновить':'⏸ Пауза'; pb.onclick=()=>send({type:'set',key:'paused',value:!st.paused});
  // log
...
function coin(c){c=c||0;return Math.floor(c/10000)+'з '+Math.floor(c%10000/100)+'с '+c%100+'м';}
function fmtDur(s){s=Math.floor(s||0);const h=Math.floor(s/3600),m=Math.floor(s%3600/60),x=s%60;return (h?h+'ч ':'')+m+'м '+x+'с';}
connect();
</script></body></html>`;
