...
function render(){
  ...
  // equipment
  document.getElementById('equip').innerHTML=(S.equip||[]).map(e=>'<tr><td class="k">'+esc(e.slotName)+'</td><td style="color:'+(QC[e.quality]||'#fff')+'">'+esc(e.name||'—')+'</td></tr>').join('')||'<tr><td class="empty">empty</td></tr>';
  // quests
  ...
  // inventory
  document.getElementById('inv').innerHTML=(S.inv&&S.inv.length)?S.inv.map(i=>'<tr><td style="color:'+(QC[i.quality]||'#fff')+'">'+esc(i.name)+' <span class="sub">'+esc(i.kindName||'')+'</span></td><td class="cnt">'+(i.count>1?('×'+i.count):'')+'</td></tr>').join(''):'<tr><td class="empty">empty</td></tr>';
