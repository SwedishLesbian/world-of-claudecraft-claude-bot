...
export class Coordinator {
  ...
  partyFormed() { const L = this.leader; if (this.bots.length === 1) return !!L?.world?.self; const n = L?.world?.self?.party?.members?.length ?? 0; return !!L?.world?.self && n >= Math.min(2, this.bots.length); }
  ...
  tick() {
    ...
    if (!this.partyFormed()) { this.phase = 'forming'; this.action = `Forming party (${(this.leader.world.self?.party?.members?.length ?? 0)}/${this.bots.length})`; for (const b of r) if (!this.inDungeon(b)) b.conn.input({}); return; }
    ...
    this.action = { leveling: `Leveling as a party (level ${partyMin})`, travel: `Traveling to ${this.target?.id ?? ''}`, dungeon: `Clearing ${this.target?.id ?? ''}`, selling: 'Selling loot on the market' }[this.phase] ?? this.action;
  ...
  driveBot(b) {
    ...
    if (self.dead) {
      ...
      b.conn.cmd({ cmd: 'release' }); b.action = '💀 Resurrecting'; return;
    }
    ...
    if (b.ctx?.settings?.paused) { b.conn.input({}); b.action = '⏸ Paused'; return; }
  ...
  driveLevel(b) {
    const L = this.leader;
    if (b === L) { b.action = '🧭 Leading the party'; decide(b.ctx); return; }
    const self = b.world.self;
    ...
    if (qa && (qa.kind === 'accept' || qa.kind === 'turnin')) { b.conn.cmd({ cmd: qa.kind, quest: qa.quest }); b.action = qa.kind === 'accept' ? '📜 Accepting quest' : '✅ Turning in quest'; return; }
    const d = dist(self, L.world.self);
    if (d > 9) { b.action = '🐾 Following the leader'; this.follow(b, L); return; }
    b.conn.input({}); b.action = '⏸ Waiting for the party';
  }
  ...
  driveTravel(b) {
    ...
    if (b === L) { if (dist(b.world.self, door) > 6) { b.action = `🧭 Traveling to ${nd.id}`; this.goTo(b, routeTo(b.world.self, door)); } else b.conn.input({}); }
    ...
    if (notIn.length && this.alive().every((x) => this.inDungeon(x) || dist(x.world.self, door) < 8)) {
      ...
      if (Date.now() - (this._enterLog || 0) > 8000) { this._enterLog = Date.now(); this.bossSeen = false; this.bossDeadAt = 0; this.dungeonDeaths = 0; this.abortDungeon = false; this.bots.forEach((x) => { x._deadCounted = false; }); this.log(`The party is entering ${nd.id}.`); }
    }
  }
  ...
  driveDungeon(b) {
    ...
    if (this.bossSeen && bossVisibleDead && !this.bossDeadAt) { this.bossDeadAt = Date.now(); this.log(`💥 BOSS DEFEATED: ${nd.boss}!`); }
    ...
    if (this.abortDungeon && !this.bossDeadAt) {
      if (this.inDungeon(b)) { b.conn.cmd({ cmd: 'leave_dungeon' }); b.action = '🚪 Retreating after wipe'; return; }
      b.conn.input({}); b.action = '⚠ Dungeon postponed';
      if (this.alive().every((x) => !this.inDungeon(x))) { this.dungeonCooldown[nd.id] = Date.now() + DUNGEON_COOLDOWN_MS; this.abortDungeon = false; this.dungeonDeaths = 0; this.bossSeen = false; this.log(`Wipe in ${nd.id} (${WIPE_DEATHS} deaths); postponing for 10 minutes and leveling.`); }
      return;
    }
    ...
    if (this.bossDeadAt) {
      if (this.inDungeon(b)) {
        ...
        b.conn.cmd({ cmd: 'leave_dungeon' }); b.action = '🚪 Leaving'; return;
      }
      b.conn.input({}); b.action = '✅ Dungeon complete';
      if (this.alive().every((x) => !this.inDungeon(x))) { this.cleared.add(nd.id); this.sellPending = this.sellEnabled; this.bossSeen = false; this.bossDeadAt = 0; this.log(`Dungeon ${nd.id} cleared${this.sellEnabled ? '; heading to market' : '; selling is disabled'}.`); }
      return;
    }

    // CLEARING: get stragglers inside
    if (!this.inDungeon(b)) { if (dist(self, nd.door) > 6) this.goTo(b, nd.door); else b.conn.cmd({ cmd: 'enter_dungeon', dungeon: nd.id }); b.action = 'Entering dungeon'; return; }

    // survival + role combat
    const hp = self.hp / Math.max(1, self.mhp);
    if (hp < 0.2 && b.role !== 'tank') { this.follow(b, this.leader); b.action = '🏃 Retreating (low HP)'; return; }
  ...
  tankCombat(b, nd) {
    ...
    if (canCast(self, { id: 'defensive_stance', learnLevel: 10, cost: 0 }) && !(self.auras ?? []).some((a) => a.kind === 'defensive_stance')) { b.conn.cmd({ cmd: 'cast', ability: 'defensive_stance' }); b.action = '🛡 Defensive stance'; return; }
    let t = self.target != null ? b.world.ents.get(self.target) : null;
    if (!t || t.dead || !t.h) t = this.nearestHostile(b, 30);
    if (!t) { b.action = '🛡 Leading toward the boss'; this.goTo(b, { x: self.x, z: self.z + 12 }); return; }
    if (b.world.dist(t) > 4) { b.action = '🛡 Approaching ' + (t.nm || t.tid); this.goTo(b, t); return; }
    ...
    if (loose && canCast(self, { id: 'taunt', learnLevel: 10, cost: 0 })) { b.conn.cmd({ cmd: 'target', id: loose.id }); b.conn.cmd({ cmd: 'cast', ability: 'taunt' }); b.conn.cmd({ cmd: 'target', id: t.id }); b.action = '🗯 Taunting'; return; }
  ...
  tryHeal(b, hurt) {
    ...
    for (const heal of this.healChain(b.cls, hurt.frac)) {
      if (canCast(self, heal)) { b.conn.cmd({ cmd: 'target', id: hurt.id }); b.conn.cmd({ cmd: 'cast', ability: heal.id }); b.action = '💚 Healing the party'; return true; }
    }
    return false;
  }
  ...
  healerCombat(b, nd) {
    ...
    if (hurt) {
      if (dist(self, hurt) > 28) { this.goTo(b, hurt); b.action = '💚 Moving to injured ally'; return; }
      if (this.tryHeal(b, hurt)) return;
    }
    // priest: keep a shield on the tank
    if (cls === 'priest') { const tk = this.leader.world.self; if (tk && tk.hp / tk.mhp < 0.92 && canCast(self, { id: 'power_word_shield', learnLevel: 6, cost: 45 })) { b.conn.cmd({ cmd: 'target', id: tk.id }); b.conn.cmd({ cmd: 'cast', ability: 'power_word_shield' }); b.action = '🛡 Shielding the tank'; return; } }
    // priest: roll Renew (HoT) on the tank — a cheap pre-heal that keeps ticking through the boss pulse.
    if (cls === 'priest') { const tk = this.leader.world.self; if (tk && Date.now() - (b._renewAt || 0) > 12000 && canCast(self, { id: 'renew', learnLevel: 8, cost: 30 })) { b.conn.cmd({ cmd: 'target', id: tk.id }); b.conn.cmd({ cmd: 'cast', ability: 'renew' }); b._renewAt = Date.now(); b.action = '🌿 Renewing the tank'; return; } }
    ...
    if (tk) {
      ...
      if (boss && dist(self, boss) < nd.aoe + 3) { this.goTo(b, this.awayFrom(self, boss, nd.aoe + 4)); b.action = '↩ Leaving the AoE'; return; }
      if (d > 24) { this.goTo(b, tk); b.action = '🐾 Staying with the party'; return; }
    }
    // light dps if safe (and out of mana-danger)
    b.conn.input({}); b.action = '💚 Ready to heal';
  }
  ...
  dpsCombat(b, nd) {
    ...
    if (!m) { this.follow(b, this.leader); b.action = '🐾 Following the party'; return; }
    ...
    if (b.world.mobsAggroOnMe().length > 0) { b.conn.cmd({ cmd: 'stopattack' }); this.goTo(b, this.leader.world.self); b.action = '⚠ Dropping aggro onto the tank'; return; }
    ...
    if (inBossAoe && (ranged || this.pulseDanger(b, nd) || self.hp / Math.max(1, self.mhp) < 0.5)) { this.goTo(b, this.awayFrom(self, m, nd.aoe + 3)); b.action = '↩ Leaving the AoE'; return; }
    if (b.world.dist(m) > range) { this.goTo(b, m); b.action = '⚔ Closing distance'; return; }
  ...
  lootNearby(b) {
    ...
    if (!c) return false;
    if (c.d > 4.5) { this.goTo(b, c.e); b.action = '👜 Moving to loot'; return true; }
    b.conn.cmd({ cmd: 'loot', id: c.e.id }); b.action = '👜 Looting'; return true;
  }

  // go to the Merchant, COLLECT prior sale proceeds, then list rare/epic loot, then resume.
  driveSell(b) {
    const self = b.world.self;
    if (dist(self, MERCHANT) > 6) { if (b === this.leader) this.goTo(b, routeTo(self, MERCHANT)); else this.follow(b, this.leader); b.action = '🛒 Going to the merchant'; return; }
    ...
    if (items.length) {
      ...
      b.action = '💰 Listing ' + (ITEMS[id]?.name || id); this.log(`${b.cls}: listed ${ITEMS[id]?.name || id} for ${SELL_PRICE[q]}c`);
      return;
    }
    b.conn.input({});
    if (this.alive().every((x) => this.sellable(x).length === 0)) { this.sellPending = false; this.log('Loot listed on the market; continuing.'); }
    b.action = '✅ Sold';
  }
}
