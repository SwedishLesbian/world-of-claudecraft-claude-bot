...
// chasing a mob can suck the bot in mid-fight. The bot steers clear of these. From content/dungeons.ts.
export const DUNGEON_DOORS = [{ x: 80, z: 90 }, { x: 45, z: 515 }, { x: 0, z: 880 }];
...
// display names (RU) for the dashboard talent panel — node id -> {ru label, max rank}
export const TALENT_INFO = {
  feral_thick_hide:       { ru: 'Толстая шкура (+броня)', max: 3 },
  feral_brutal_impact:    { ru: 'Жестокий удар (Maul/Swipe)', max: 2 },
  feral_feline_swiftness: { ru: 'Кошачья ловкость (+уворот)', max: 2 },
  feral_choice:           { ru: 'Звериный инстинкт', max: 1 },
  feral_heart_wild:       { ru: 'Сердце дикой природы', max: 2 },
  feral_ferocity:         { ru: 'Свирепость (дешевле Maul)', max: 3 },
  // paladin (retribution)
  ret_benediction:        { ru: 'Благословение (дешевле Печать/Правосудие)', max: 3 },
  ret_seal_command:       { ru: 'Печать Воли (+урон Печати)', max: 2 },
  ret_imp_judgement:      { ru: 'Улучш. Правосудие (−КД, +урон)', max: 2 },
  ret_choice:             { ru: 'Путь Воздаятеля', max: 1 },
  ret_crusader_strikes:   { ru: 'Удары крестоносца (+урон)', max: 2 },
  ret_conviction:         { ru: 'Убеждённость (+крит)', max: 3 },
};
export const TALENT_CHOICE_RU = {
  feral_choice_survival: 'Инстинкт выживания (+14% HP)', feral_choice_bear: 'Лютый медведь (+броня)', feral_choice_cat: 'Хищные удары (+урон)',
  ret_choice_pursuit: 'Жажда правосудия (+10% сила атаки, +уворот)', ret_choice_sanctity: 'Аура святости (+урон заклинаний)', ret_choice_vengeance: 'Возмездие (+крит)',
};
export const SPEC_RU = { feral: 'Ферал (медведь)', balance: 'Баланс', restoration: 'Восстановление', retribution: 'Воздаяние (DPS)', holy: 'Свет (хил)', protection: 'Защита (танк)' };
