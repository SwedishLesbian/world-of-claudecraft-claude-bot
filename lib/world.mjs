...
export class World {
  ...
  threatWeight(m, myLv) {
    const gap = myLv - (m.lv ?? myLv);     // >0 ⇒ the mob is BELOW us (weaker); <0 ⇒ ABOVE us (harder)
    if (gap >= 5) return 0;                // grey  — trivial, facerolled, free
    if (gap >= 3) return 0.5;              // green — easy
    if (gap >= -2) return 1;               // yellow / at-level / +1..+2 — a fair fight
    if (gap >= -4) return 1.75;            // orange (+3..+4) — slow, dangerous: counts nearly double
    return 3;                              // red (+5 and up) — brutal: one alone already blows a 2-cap budget
  }
