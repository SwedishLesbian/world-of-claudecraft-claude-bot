...
import { MOB_DENSITY } from './density.generated.mjs';
...
export class World {
  ...
  // Σ threatWeight over the EXTRA mobs a pull drags in (target excluded — that's engageCost's job). Scaled by
  // the target camp's PACK DENSITY: predictPull only sees the mobs in aggro range NOW, but a tight pack
  // (deepfen_murloc: 8 in radius 15) wanders more in mid-fight than that snapshot shows, so the raw cascade
  // under-reads — the static density multiplier (>=1, from gen_bot_density.mjs) compensates so a dense camp
  // costs more and the bot waits to out-level it instead of committing to a "winnable" single that 5-pulls.
  brawlLoad(target, myLv) {
    const raw = this.predictPull(target, myLv).reduce((s, m) => s + this.threatWeight(m, myLv), 0);
    return raw * (MOB_DENSITY[target.tid] ?? 1);
  }
