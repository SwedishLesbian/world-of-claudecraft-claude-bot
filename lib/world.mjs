...
export class World {
  ...
  // THREAT WEIGHT of a would-be joiner. The pull model counts bodies, but a body well BELOW our level is
  // cheap — a plate/heal class cleaves grey/green adds without trouble, and the game agrees (xp + threat scale
  // with the level gap). So weight each joiner by how dangerous it ACTUALLY is, not 1-per-head: this is what
  // lets the bot BRAWL a pack of out-levelled mobs (do the easy low-level quests it used to refuse as "too
  // dense") while still refusing equal/over-level density — the real killer. (An over-LEVEL mob, gap<0, is full
  // weight, so an under-levelled bot in a too-high camp stays correctly protected.)
  threatWeight(m, myLv) {
    const gap = myLv - (m.lv ?? myLv);     // >0 ⇒ the mob is BELOW us (weaker)
    if (gap >= 5) return 0;                // grey: trivial threat, facerolled — doesn't count against capacity
    if (gap >= 3) return 0.5;              // green: easy
    return 1;                              // yellow / at-level / above: full threat
  }
  // Weighted brawl LOAD of pulling `target` = Σ threatWeight over its predicted joiners (target excluded).
  brawlLoad(target, myLv) { return this.predictPull(target, myLv).reduce((s, m) => s + this.threatWeight(m, myLv), 0); }

  // Cleanest instance among candidates: lightest weighted brawl LOAD, then nearest. Returns {m, load} or null.
  cleanestPull(cands, myLv) {
    let best = null, bestKey = Infinity;
    for (const m of cands) {
      const load = this.brawlLoad(m, myLv);
      const key = load * 1000 + this.dist(m);                        // lightest brawl load dominates, then distance
      if (key < bestKey) { bestKey = key; best = { m, load }; }
    }
    return best;
  }
  // Accept a pull whose weighted LOAD is within our BRAWL capacity (maxJoin = combatCap-1). Body-COUNT is no
  // longer the gate — difficulty is — so a pack of weak out-levelled mobs is taken (the flee rule still fights
  // it: a grey add adds ~0 real pressure), while equal/over-level density is refused. Heavier → null (relocate).
  acceptable(best, myLv, maxJoin = 0) {
    if (!best) return null;
    return best.load <= maxJoin ? best.m : null;
  }
